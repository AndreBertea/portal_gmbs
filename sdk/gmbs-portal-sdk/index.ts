 ...
 *   keyId: process.env.GMBS_PORTAL_KEY_ID,
 *   secret: process.env.GMBS_PORTAL_SECRET
 * })
 * 
 * // Check subscription
...
export class GMBSPortalSDK {
   ...
   * 
   * @param params - Artisan ID and optional metadata
   * @returns Token and portal URL (token is only returned once!)
   */
   ...
   * 
   * @param ids - Array of submission IDs to mark as synced
   */
   ...
   * 
   * @param token - The raw portal token
   */
  async validateToken(token: string): Promise<{
    ...
    // This endpoint doesn't require auth headers
    const url = `${this.baseUrl}/tokens/${token}/validate`
    const res = await fetch(url)
    return res.json()
  }
}
