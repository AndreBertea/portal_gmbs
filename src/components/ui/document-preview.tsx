...
export function DocumentPreview({ url, mimeType, filename, className }: DocumentPreviewProps) {
  ...
  if (previewType === "pdf") {
    return (
      <div className={cn("overflow-hidden rounded-md border border-slate-200 min-h-[10rem]", className)}>
        <iframe
          src={url}
  ...
  return (
    <div
      className={cn(
        "flex min-h-[10rem] flex-col items-center justify-center gap-3 rounded-md border border-slate-200 bg-slate-50 p-4 text-center",
        className
      )}
    >
